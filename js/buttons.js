...
function handleLoaded() { $progress.max = $video.duration; }
function handleTimeUpdate() { $progress.value = $video.currentTime; }

$progress.addEventListener('input', handlInput)
function handlInput() { $video.currentTime = $progress.value; }
